...
| 身分 | 取得方式 | 訂房價格 | 「會費核對狀態」欄的值 |
|------|------|------|------|
| 一般會員 | 網頁免費註冊 | 原價 | `一般會員` |
| 訂閱制會員 | 匯款 3,000 年費（轉儲值金）後由管家開通 | **8 折**＋儲值扣款 | `待確認` → `已開通` |

- **訂房一律需要會員帳號**（手機號碼登入）
- 一般會員匯款回報後自動變 `待確認`，管家對帳改成 `已開通` 並填入「到期月份」即完成升級
- 到期月份過期後，網頁自動以原價計費並提示續約（試算表不用改）
...
```json
{ "action": "register", "data": { "name": "陳美惠", "lineName": "美惠媽咪", "phone": "0912345678" } }
```

成功回傳新會員資料（`status: "一般會員"`）；手機重複時回 `{ ok: false, message: "此手機號碼已註冊過…" }`。
